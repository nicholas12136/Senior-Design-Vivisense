...
void printPointCsv(uint32_t timestampMs, const Point &p)
{
  ...
  Serial.print(",");
  Serial.print(p.cellId);
  Serial.print(",");
  Serial.print(p.worldX, 1);
  Serial.print(",");
  Serial.print(p.worldY, 1);
  Serial.print(",");
  Serial.print(p.worldZ, 1);
  Serial.print(",");
  Serial.print(p.targetStatus);
  Serial.print(",");
...
void loop()
{
  ...
  if (allReady)
  {
    ...
    for (int i = 1; i < NUM_SENSORS; i++)
      if (lastTimestampMs[i] > frameTs)
        frameTs = lastTimestampMs[i];
